...
class _CameraScanScreenState extends State<CameraScanScreen>
  ...
  void _showHowToScan() {
    showModalBottomSheet<void>(
      ...
      builder: (context) => const _HowToScanSheet(),
    );
  }
  ...
  double calculateStringSimilarity(String a, String b) {
    ...
    for (final aWord in aWords) {
      for (final bWord in bWords) {
        ...
        if (aWord.length > 3 && bWord.length > 3) {
          final distance = _levenshteinDistance(aWord, bWord);
    ...
    // = 2 / 2 = 1.0
    final productWordCoverage = matchingWords / bWords.length;

    // Also calculate OCR coverage.
    final ocrWordCoverage = matchingWords / aWords.length;
    ...
    // "Dairy Milk".
    return math.min(productWordCoverage, ocrWordCoverage);
  }
  ...
  List<String> extractOcrCandidates(RecognizedText recognizedText) {
    ...
    for (int i = 0; i < ocrLines.length; i++) {
      ...
      if (i + 2 < ocrLines.length) {
        final combined = '${ocrLines[i]} ${ocrLines[i + 1]} ${ocrLines[i + 2]}';
        if (combined.length <= 50) {
          candidates.add(combined);
        }
      }
  ...
  Future<void> _capture() async {
    ...
    try {
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text('Could not process image: $e')));
    } finally {
      ...
      if (mounted &&
        ...
        try {
          ...
          await currentCamera.startImageStream(_processCameraImage);
        } catch (e) {
  ...
  Future<void> _lookupFood(String barcode, XFile productImage) async {
    _setProcessingState(
      true,
      title: '🔍 Analyzing your product…',
      subtitle: 'Checking nutrition, ingredients & personalized compatibility.',
    ...
    try {
      ...
      _setProcessingState(
        ...
        title: '🔍 Analyzing your product…',
        subtitle:
            'Checking nutrition, ingredients & personalized compatibility.',
      );
      ...
      if (product != null) {
        ...
        await Navigator.push(
          ...
          MaterialPageRoute(
            builder: (_) => AiAnalysisScreen(
              capturedImage: FileImage(File(productImage.path)),
              product: product,
              productName: product.name,
      ...
      if (ocrProduct != null) {
        ...
        ScanHistoryService.instance.saveScan(ocrProduct);
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text('Error processing product: $e')));
  ...
  Future<void> _processCameraImage(CameraImage image) async {
    if (_isProcessingImage || _barcodeDetected || _isManualCapturing) {
      return;
    }
                        ...
                        children: [
                          ...
                          Positioned(
                            ...
                            child: _SideActions(
                              ...
                              onHowToScanTap:
                                  widget.onHowToScanTap ?? _showHowToScan,
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          _FlashPill(uiScale: uiScale, active: flashOn, onTap: onFlashToggle),
        ],
      ),
    );
  }
          ...
          child: Row(
            ...
            children: [
              Icon(
                ...
                color: widget.active ? const Color(0xFF5CE0A0) : Colors.white70,
              ),
        ...
        children: [
          Container(
            ...
            decoration: BoxDecoration(
              ...
              border: Border.all(color: const Color(0xFF9B7BFA), width: 1.2),
            ),
          ...
          GestureDetector(
            onTap: onDismiss,
            child: Icon(Icons.close, size: 16 * uiScale, color: Colors.white54),
          ),
        ],
      ),
    );
        ...
        return Stack(
          children: [
            ...
            AnimatedBuilder(
              ...
              builder: (context, _) {
                final y =
                    16 + scanLineCtrl.value * (constraints.maxHeight - 32);
                return Positioned(
                  left: 8,
                  right: 8,
                  top: y,
...
class _HowToScanSheet extends StatelessWidget {
  const _HowToScanSheet();

  static const _background = Color(0xFF111126);
  static const _purple = Color(0xFF9B7BFA);
  static const _green = Color(0xFF57D39B);
    ...
    return SafeArea(
      ...
      child: Container(
        ...
        decoration: const BoxDecoration(
          ...
          border: Border(top: BorderSide(color: Color(0xFF3B3962))),
        ),
          ...
          child: Column(
            ...
            children: [
                  ...
                  const Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        SizedBox(height: 6),
                        Text(
                          'Choose the best way to identify and analyze your product',
                          style: TextStyle(
              ...
              const _ScanMethodCard(
                ...
                badge: 'RECOMMENDED',
                description:
                    'Scan the product barcode for fast and accurate product identification.',
                steps: [
                  'Place the barcode inside the scanning frame.',
                  'Keep the barcode steady and clearly visible.',
                  'The barcode is used to find the exact product from the product database.',
                ],
                supportingLabel: 'Best for exact product identification',
                accent: _green,
                background: Color(0xFF142B27),
              ),
              const SizedBox(height: 12),
              const _ScanMethodCard(
                icon: Icons.document_scanner_outlined,
                title: '2. Scan Product Label',
                badge: 'AI / OCR',
                description:
                    'No barcode? Scan the product label and AI will read the information for you.',
                steps: [
                  'Keep the product label inside the scanning frame.',
                  'Make sure the text is clear and well-lit.',
                  'AI/OCR can extract the product name, ingredients, and nutrition information.',
                ],
                supportingLabel:
                    "Useful when the barcode isn't available or isn't recognized",
                accent: _purple,
                background: Color(0xFF211B3B),
              ),
              const Padding(
                padding: EdgeInsets.symmetric(vertical: 20),
                child: Divider(color: Color(0xFF353452), height: 1),
              ),
              const Text(
                'Which should I use?',
                style: TextStyle(
              ...
              const Text(
                'Barcode scanning is recommended when available because it provides faster and more reliable product identification.\n\nUse Label Scan (OCR) when the barcode cannot be scanned or the product isn\'t found.',
                style: TextStyle(
                  color: Color(0xFFB9B7D0),
                  fontSize: 13.5,
                  height: 1.45,
