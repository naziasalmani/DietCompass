...
void main() {
  ...
  group('DietCompass Recipe Generator & Product Context Flow Tests', () {
    test('RecipeCardData properly binds full Recipe model with strict ingredient preservation', () {
      ...
      final hasProductInIngs = fullRecipe.ingredients.any(
        (i) => i.name.toLowerCase().contains('dairy milk') || i.name.toLowerCase().contains('cadbury'),
      );
      expect(hasProductInIngs, isTrue, reason: 'Source product must appear in ingredients list');

      // Verify product is in instructions
      final hasProductInSteps = fullRecipe.instructions.any(
        (step) => step.toLowerCase().contains('dairy milk') || step.toLowerCase().contains('chocolate'),
      );
      expect(hasProductInSteps, isTrue, reason: 'Source product must appear in recipe instructions');
    });

    test('RecipeCardData properly binds Maggi Masala Noodles in ingredient and instruction lists', () {
      const maggiCard = RecipeCardData(
        ...
        fullRecipe: Recipe(
          ...
          nutritionFacts: [],
          ingredients: [
            IngredientItem(amount: '1 pack', name: 'Maggi Masala Noodles with Tastemaker'),
    ...
    testWidgets('BUG 1 & 2 FIX: Recipe Card opens EXACT SAME Recipe Detail with sourceProduct context', (tester) async {
    ...
    testWidgets('Normal Recipe Generator mode opens without product and maintains 1-to-1 card consistency', (tester) async {
      tester.view.physicalSize = const Size(1200, 2600);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(tester.view.resetPhysicalSize);
      ...
      const normalRecipe = RecipeCardData(
        ...
        recipeSource: 'spoonacular',
        whatsInside: [
          WhatsInTag(icon: Icons.eco_rounded, title: 'Antioxidants', subtitle: 'Fresh berries', color: Color(0xFF1E8A4C)),
        ],
        fullRecipe: Recipe(
          id: 'recipe_smoothie_1',
